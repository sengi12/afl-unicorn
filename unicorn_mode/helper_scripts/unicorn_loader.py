...
# Optional: drcov coverage output for ghidra-aflcov / Lighthouse / Dragondance.
# Kept optional so the loader still imports if drcov.py is not alongside it.
try:
    from drcov import DrcovWriter
except ImportError:
    DrcovWriter = None
...
class UnicornSimpleHeap(object):
    ...
    def realloc(self, ptr, new_size):
        ...
        if old_chunk != None:
            self._uc.mem_write(new_chunk_addr, bytes(self._uc.mem_read(old_chunk.data_addr, old_chunk.data_size)))
...
class AflUnicornEngine(Uc):
    ...
    def __init__(self, context_directory, enable_trace=False, debug_print=False, enable_coverage=False):
        """
        ...
        Parameters:
          ...
          - enable_coverage: If True, executed basic blocks are recorded so dump_coverage()
                             can write a drcov file for a coverage visualizer.
        """

        # Coverage state. Set before mapping segments so __map_segments can note
        # the span of the executable segments (a sensible default module range).
        self._context_directory = context_directory
        self._coverage_enabled = enable_coverage
        self._coverage_blocks = set()
        self._exec_min = None
        self._exec_max = None
        ...
        if enable_coverage:
            self.hook_add(UC_HOOK_BLOCK, self.__collect_block)
    ...
    def dump_coverage(self, filename, base=None, end=None, module_name=None):
        """
        Write the basic-block coverage collected during emulation to a drcov
        file, readable by ghidra-aflcov, Lighthouse and Dragondance.

        Requires the engine to have been created with enable_coverage=True.

        Parameters:
          - filename: where to write the drcov file
          - base/end: bound the module of interest. Blocks outside [base, end)
                      are dropped and offsets are stored relative to base, so
                      they line up with the binary's RVAs in the disassembler.
                      Defaults to the span of the loaded executable segments.
          - module_name: name recorded in the drcov module table; a disassembler
                      matches it against the open program. Defaults to "target".

        Returns the number of unique blocks written.
        """
        if DrcovWriter is None:
            raise Exception("drcov.py was not found next to unicorn_loader.py; cannot write coverage")
        if not self._coverage_enabled:
            raise Exception("coverage was not collected; construct the engine with enable_coverage=True")
        if base is None:
            base = self._exec_min
        if end is None:
            end = self._exec_max
        if base is None or end is None or end <= base:
            raise Exception("could not determine the module range; pass base= and end= explicitly")
        if module_name is None:
            module_name = "target"

        writer = DrcovWriter()
        mod_id = writer.add_module(module_name, base, end)
        for address, size in self._coverage_blocks:
            if base <= address < end:
                writer.add_block(mod_id, address - base, size)
        writer.save(filename)
        return writer.block_count()
    ...
    def __map_segments(self, segment_list, context_directory, debug_print=False):
        for segment in segment_list:
            ...
            # Remember the span of executable memory: it is the default module
            # range for coverage output when the harness does not specify one.
            if perms & UC_PROT_EXEC:
                if self._exec_min is None or seg_start < self._exec_min:
                    self._exec_min = seg_start
                if self._exec_max is None or seg_end > self._exec_max:
                    self._exec_max = seg_end
    ...
    def __collect_block(self, uc, address, size, user_data):
        # UC_HOOK_BLOCK fires once per basic block entered. Record the absolute
        # address here; rebasing to a module offset happens in dump_coverage,
        # which lets the harness choose the module range after the run.
        self._coverage_blocks.add((address, size))
